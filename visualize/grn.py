...
attention, vq_code, gene_indexes = torch.load('pancreas2.pth')
indexes = (vq_code == 820).cpu()
attention = attention.mean(1).cpu()
...
for edge in sorted_edges[:50]:
    weight = edges[edge]
    G.add_edge(edge[0], edge[1], weight=weight/20)

# 计算边的权重，设置边的宽度
weights = [G[u][v]['weight'] for u, v in G.edges()]

# 绘制图形
pos = nx.spring_layout(G, k=1)  # 选择布局
plt.figure(figsize=(12, 8))
nx.draw_networkx_nodes(G, pos, node_size=700, node_color='none', edgecolors='none')  # edgecolors='none'去掉边框
nx.draw_networkx_edges(G, pos, width=weights, edge_color='violet')
nx.draw_networkx_labels(G, pos, font_size=15, font_family='sans-serif')

# 添加边权重标签
edge_labels = nx.get_edge_attributes(G, 'weight')
filtered_edge_labels = {k: round(edges[k]/20, 3) for k in sorted_edges[:10]}
nx.draw_networkx_edge_labels(G, pos, edge_labels=filtered_edge_labels, font_size=10)

plt.title("State 820 genes interaction", fontsize=30)
plt.axis('off')  # 关闭坐标轴
plt.tight_layout()
plt.savefig('figures/grnre2_820.pdf')