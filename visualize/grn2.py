...
attention, vq_code, gene_indexes = torch.load('pancreas.pth')
indexes = (vq_code == 820)
attention = attention.mean(1).cuda()
attention = attention[indexes][:, 0, 1:]
gene_indexes = gene_indexes[indexes]
attention = scatter_sum(attention.cpu(), dim=-1, index=gene_indexes.cpu()).mean(0)
...
for i, gene in enumerate(topk_genes):
    G.add_edge('State 820', gene, weight=round(float(values[i]),4))
...
nx.draw_networkx_edges(G, pos, width=weights*5, edge_color='violet')
...
plt.title("State 820 state-gene interaction", fontsize=30)
plt.axis('off')  # 关闭坐标轴
plt.tight_layout()
plt.savefig('figures/stategene_820.pdf')