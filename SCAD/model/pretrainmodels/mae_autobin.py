...
class MaeAutobin(nn.Module):
    ...
    #    self.cell_vq = HierCVQLayer(14, embed_dim, latent_dim, levels=[14], condition_layer=condition_layer, num_codebooks=1)
    ...
    def forward(self, x, 
       ...
       # with torch.no_grad():
        ...
        grn_emb = self.grn_embed(self.grn[encoder_position_gene_ids])
        ...
        mean = torch.clamp(torch.exp(self.mean(x).squeeze(2)), min=1e-5, max=1e6)
        pi = self.pi(x).squeeze(2).sigmoid()
        disp = torch.clamp(F.softplus(self.disp(x).squeeze(2)), min=1e-4, max=1e4)
        x = self.to_final(x)
        return x.squeeze(2), mean, pi, disp, cell_type, tissue, disease, sex, age
    

    def get_cellemb(self, geneemb, encoder_position_gene_ids, padding_label):
        geneemb1 = geneemb[:,-1,:]
        try:
            geneemb2 = geneemb[:,-2,:]
