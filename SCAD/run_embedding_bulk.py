...
parser.add_argument('--data_path', type=str, default='./data/split_norm/Source_exprs_resp_19264.Etoposide.csv', help='encoder or all')
...
def main():
    ...
    pretrainmodel,pretrainconfig = load_model_frommmf('../model/models/models.ckpt', key='cell' ,device=device)
    params = torch.load('../FoldToken4/results/maskgene_finetune/checkpoints/best_model.pt', map_location='cpu')
    params = {key.replace('_forward_module.model.',''):val for key, val in params.items()}
    pretrainmodel.load_state_dict(params, strict=False)
    pretrainmodel.eval()
    geneexpemb=[]
    
    strname = 'data/split_norm/' +  args.data_path[:-4].split('/')[-1] +'_' + args.ckpt_name
    ...
    for i in tqdm(range(gexpr_feature.shape[0])):
        with torch.no_grad():
            if pretrainconfig['rawcount'] == False:
                ...
                target = np.full_like(totalcount, 4)
                tmpdata = gexpr_feature.iloc[i,:].tolist()
                pretrain_gene_x = torch.tensor(tmpdata+[target,totalcount]).unsqueeze(0).cuda()
            ...
            if args.type=='encoder':
                ...
                geneemb = pretrainmodel.encode(x.float(), x_padding, position_gene_ids)
