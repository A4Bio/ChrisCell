...
class HierCVQLayer(nn.Module):
    def __init__(self, log2_num_embeddings, embedding_dim, vq_dim, levels, condition_layer=6, num_codebooks=1, sphere=True):
        ...
        self.embedding_mlp = nn.Sequential(self.build_condition_layer(log2_num_embeddings,hidden_dim,condition_layer,vq_dim))
        self.gene_mlps = nn.Parameter(torch.randn(num_codebooks, vq_dim, vq_dim))
    ...
    def attention(self, H, C, temperature=1):
        alpha = 1/temperature
        mat = torch.einsum('kj, kij->ki', H, C)
        distances = - 2 * (alpha-1)* mat.detach() - 2 * mat
...
# print(FT3_val.mean(), FT3_val.min(), FT3_val.max())
    ...
    def sample_code(self, h, level, temperature, mask=None, attn_mask=None):
        bvectors = self.get_bvectors(level).to(h.dtype)
        bvectors = F.pad(bvectors, (self.log2_num_embeddings-level,0,0,0), value=-1).to(h.device)

        embed = self.embedding_mlp(bvectors)
        if mask is not None:
            weights = self.gene_mlps[mask]
            embed = torch.einsum('ij, kjj->kij', embed, weights)
        else:
        ...
        c = code_one_hot*(1-A) + (1-code_one_hot)*(-A)
        A2 = A + c.detach()
        A2 = torch.einsum('ki, kij->kj', A2, embed)
        h_vq = h_flat + A2 - h_flat.detach()
        

        quantized = torch.zeros_like(h)
        quantized[attn_mask==1] = h_vq
        return quantized, code, vq_loss, h


    def forward(self, h_in, attn_mask=None, mode='train', temperature=1, mask=None, vqshortcut=False, level = 8):
        h = self.proj(h_in)
        if h.isnan().any():
            h = torch.rand_like(h)
            # print(h)
        ...
        #        print(level)
                quantized_, code, vq_loss_, embed = self.sample_code(h, level, temperature, mask=mask, attn_mask=attn_mask)
            ...
            print(level)
            quantized, code, vq_loss, embed = self.sample_code(h, level, temperature, mask=mask, attn_mask=attn_mask)
        ...
        return quantized, vq_code, vq_loss, embed
