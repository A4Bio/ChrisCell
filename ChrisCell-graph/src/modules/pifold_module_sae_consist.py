...
class GeoFeaturizer(nn.Module):
    ...
    def construct_virtual_frame(self, T, batch_id, num_global=3):
        ...
        # R = torch.eye(3, device=X_m.device, dtype=X_m.dtype)[None].repeat(X_m.shape[0],1,1)
        ...
        # T_gs._rots._rot_mats = torch.eye(3, device=X_m.device, dtype=X_m.dtype)[None,None].repeat(T_gs.shape[0],1,1,1)
        # T_gs._trans = torch.zeros(3, device=X_m.device, dtype=X_m.dtype)[None,None].repeat(T_gs.shape[0],1,1)
