...
class PretrainInterface(torch.nn.Module):
    def __init__(self, name):
        ...
        if name == 'ProGLM_SoftCVQ_CATH_epoch10k':
            ...
            self.pretrain_model = MInterface(**pretrain_args)
            ckpt = torch.load('/huyuqi/xmyu/VQProteinFormer/step2_ProGLM/results/ProGPT_SoftCVQ_CATH_epoch20000_resume/checkpoints/best-epoch=19959-gpt_loss=0.048.pth')
            state_dict = {k.replace('_forward_module.', ''): v for k, v in ckpt.items()}
            self.pretrain_model.load_state_dict(state_dict)

        if name == 'ProGLM_SoftCVQ_uncond':
            from step4_ProGLM_uncond.model.model_interface import MInterface
            pretrain_args = OmegaConf.load("/huyuqi/xmyu/VQProteinFormer/step4_ProGLM_uncond/results/ProGPT_uncond_SoftCVQ_CATH_15layer_hsize480/configs/03-27T16-43-20-project.yaml")
            self.pretrain_model = MInterface(**pretrain_args)
            ckpt = torch.load('/huyuqi/xmyu/VQProteinFormer/step4_ProGLM_uncond/results/ProGPT_uncond_SoftCVQ_CATH_15layer_nolength_embed/checkpoints/best-epoch=6316-gpt_loss=0.051.pth')
            state_dict = {k.replace('_forward_module.', ''): v for k, v in ckpt.items()}
            self.pretrain_model.load_state_dict(state_dict)
