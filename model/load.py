...
import numpy as np
import random
from model.pretrainmodels import select_model
import math
...
def load_model_frommmf(best_ckpt_path, mode='m1', params=None, device='cuda'):
    model_data = torch.load(best_ckpt_path,map_location='cpu')
    config = model_data['config']
    model_data = model_data[mode]
    ...
    if params is not None:
        ...
        config['condition_layer'] = params['condition_layer']
        config['latent_dim'] = params['latent_dim']
    model = select_model(config)
...
def getEncoerDecoderData(data, data_raw, config):
    ...
    encoder_data_labels = (data_raw > 0) & (data_raw != float(config['mask_token_id']))
