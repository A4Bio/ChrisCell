...
class GEARS_Model(torch.nn.Module):
    ...
    def sample_data(self, bs, length, num):
        index = np.zeros([bs, length])
        for i in range(bs):
            a = np.random.choice(np.arange(length), replace=False, size=num)
            index[i][a] = True
        return index
    ...
    def forward(self, data):
        ...
        if self.no_perturb:
            ...
            if self.pretrained:
                ...
                emb = self.gene_emb(torch.LongTensor(list(range(self.num_genes))).repeat(num_graphs, ).to(self.args['device'])).reshape(num_graphs, self.num_genes, -1)
                sample_index = torch.from_numpy(self.sample_data(x.shape[0], self.num_genes, 5000)).to(self.args['device']).to(torch.bool)
                sc_emb = self.singlecell_model(pre_in, sample_index) 
                emb[sample_index] = sc_emb.view(-1, sc_emb.shape[-1])
