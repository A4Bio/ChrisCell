...
class GEARS:
    ...
    def train(self, epochs = 20, result_dir='./results',
        ...
        print('training steps:', len(train_loader))
        for epoch in tqdm(range(epochs)):
            self.model.train()
            if self.config['finetune_method'] == 'frozen' and self.config['model_type']:
        ...
        #    test_metrics, _ = compute_metrics(test_res)
            ...
            log = "Epoch {}: Test Overall MSE: {:.4f} " \
                  "Validation Overall MSE: {:.4f}. "
            print_sys(log.format(epoch + 1, test_metrics['mse'], 
                             val_metrics['mse']))
            
            # Print epoch performance for DE genes
            log = "Test Top 20 DE MSE: {:.4f} " \
                  "Validation Top 20 DE MSE: {:.4f}. "
            print_sys(log.format(test_metrics['mse_de'],
                             val_metrics['mse_de']))
            
            # Print epoch performance
            log = "Epoch {}: Test Overall Pearson: {:.4f} " \
                  "Validation Overall Pearson: {:.4f}. "
            print_sys(log.format(epoch + 1, test_metrics['pearson'], 
                             val_metrics['pearson']))
            
            # Print epoch performance for DE genes
            log = "Test Top 20 DE Pearson: {:.4f} " \
                  "Validation Top 20 DE Pearson: {:.4f}. "
            print_sys(log.format(test_metrics['pearson_de'],
                             val_metrics['pearson_de']))

            if self.wandb:
                metrics = ['mse', 'pearson']
                for m in metrics:
                    self.wandb.log({'train_' + m: test_metrics[m],
                               'val_'+m: val_metrics[m],
                               'train_de_' + m: test_metrics[m + '_de'],
