...
def choose_binset(full_seq, bin_set, bin_class,count_num=5,highres=0):
    if bin_set == 'bin_2':
        ...
        targetindx = torch.ones_like(inputindex) * 4
        full_seq = torch.cat((full_seq, targetindx,inputindex),dim=1) 
    else:
...
class MAEAutobinencoder(nn.Module):
    ...
    def init_module(self):
        ...
        self.m.load_state_dict(params, False)
    ...
    #    x = choose_binset(x, self.config['bin_set'], self.num_tokens,highres=self.config['highres'])
        encoder_data = x[:, :-1][sample_index].view(-1, 5000)
        encoder_position_gene_ids = torch.arange(19264)[None, :].repeat(x.shape[0], 1).to(x.device)
        encoder_position_gene_ids = encoder_position_gene_ids[sample_index].view(-1, 5000)
    #    encoder_data, encoder_position_gene_ids, encoder_data_padding, encoder_labels, decoder_data, decoder_data_padding, new_data_raw, data_mask_labels, decoder_position_gene_ids = getEncoerDecoderData(x,x,self.model_config)
        ...
        out = self.m.forward(x=encoder_data, padding_label=None,
