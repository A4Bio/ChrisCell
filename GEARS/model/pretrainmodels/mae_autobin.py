...
class MaeAutobin(nn.Module):
    def __init__(
        ...
        # ## DEBUG
        self.encoder = None
        self.gene_vq = HierCVQLayer(6, decoder_embed_dim, 128, levels=[6], condition_layer=3, num_codebooks=19266)
        ...
        self.to_decoder = nn.Linear(decoder_embed_dim, decoder_embed_dim)

        # GRN
        self.grn = torch.load('grn.pth')
    ...
    #    decoder_data = torch.matmul(self.grn, decoder_data) # [B,N,D]
            x = self.decoder(decoder_data, padding_mask=decoder_data_padding_labels)
            x = self.norm(x)

        gene_vq, gene_vqcode, gene_vqloss, h_G_embed = self.gene_vq(x.view(-1, x.shape[-1]), temperature=temperature, mode=mode, level=level, mask=decoder_position_gene_ids.view(-1), vqshortcut=vqshortcut)
        gene_vq = torch.matmul(self.grn, gene_vq.view(b, -1, gene_vq.shape[-1])) # [B,N,D]
        gene_vq = self.to_decoder(gene_vq)
        
        return gene_vq, x, gene_vqcode, gene_vqloss